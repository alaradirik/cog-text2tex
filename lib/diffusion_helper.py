...
def get_inpainting(device):
    ...
    model = StableDiffusionInpaintPipeline.from_pretrained(
        "stable-diffusion-2-inpainting-cache",
...
def apply_controlnet_depth(
    ...
    if blend > 0 and transforms.ToTensor()(keep_mask_image).sum() > 0:
        ...
        diffused_image = Image.fromarray(generate_image_np.astype(np.uint8)).convert(
            "RGB"
        )
...
def apply_inpainting(
    model, init_image, mask_image_tensor, prompt, height, width, device
):
    """
    Use Stable Diffusion 2 to generate image
...
@torch.no_grad()
def apply_inpainting_postprocess(
    model, init_image, mask_image_tensor, prompt, height, width, device
):
    """
    Use Stable Diffusion 2 to generate image
    ...
    init_images_tensor = diffused_image_tensor * mask_image_tensor[
        0
    ] + init_images_tensor * (1 - mask_image_tensor[0])
    init_image = Image.fromarray(
        init_images_tensor.cpu().numpy().astype(np.uint8)
    ).convert("RGB")
