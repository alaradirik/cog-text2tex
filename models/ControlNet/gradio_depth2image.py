...
def init_model():
    ...
    state_dict = load_state_dict("controlnet-depth-cache/control_sd15_depth.pth")
...
def process(
    ...
    with torch.no_grad():
        ...
        ddim_sampler.make_schedule(
            ddim_num_steps=ddim_steps, ddim_eta=eta, verbose=False, strength=strength
        )
        ddim_steps = int(ddim_steps * strength)  # actually DEPRECATED

        # add noises to the maximum
        ddim_steps_tensor = torch.full(
            (x0.shape[0],), ddim_sampler.ddim_timesteps[-1]
        ).to(model.device)
        ...
        detected_map_resized = cv2.resize(
        ...
        if unknown_mask is not None:
            ...
            detected_map_image = Image.fromarray(detected_map.astype(np.uint8)).convert(
                "L"
            )
            ...
            unknown_mask_image = (unknown_mask * 255.0).astype(np.uint8)
        ...
        unknown_mask_dilate = Image.fromarray(
            unknown_mask_dilate.astype(np.uint8)
        ).convert("L")
        unknown_mask_dilate = unknown_mask_dilate.resize(
            (H // 8, W // 8), Image.NEAREST
        )
        unknown_mask_dilate = transforms.ToTensor()(unknown_mask_dilate).to(
            model.device
        )
        unknown_mask_dilate = unknown_mask_dilate.repeat(4, 1, 1)
        ...
        try:
            assert set(
                torch.unique(unknown_mask_dilate).cpu().numpy().tolist()
            ).issubset(set([0, 1])), set(
                torch.unique(unknown_mask_dilate).cpu().numpy().tolist()
            )
